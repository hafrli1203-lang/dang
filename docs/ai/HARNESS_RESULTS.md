...
### [2026-06-21] UI 개선 — 생성물 마크다운 → 문서급 렌더 — Trigger: 사용자 "마크다운만 뿌려 보기 힘들다, 문서처럼"
- 프로파일: UI(CSS 전용, dg-prose 한정)
- 수정: `theme_css.py` `.dg-prose` 문서급 타이포그래피 강화(한 곳 → 보고서·기획·소식글 전 페이지 적용). 제목 위계(h1~h4)·문단 리듬·목록(오렌지 마커)·인용(좌측바+연오렌지)·표(라운드/그림자/제브라/hover)·코드/pre·구분선·강조·링크·본문 80ch 측정폭(표·코드·이미지는 전체폭)·한글 keep-all. --dg-* 변수만 사용(테마 자동 적응).
- 게이트: **402 passed**(회귀 0). compile OK. **라이브 렌더 스크린샷**으로 문서화 확인(H1 타이틀·오렌지 섹션 헤더+밑줄·표 제브라·인용 하이라이트, 80ch가 표 안 깨뜨림).
- 검수: ux-harness-reviewer=추가작업 없음(P0/P1 미제기).
- 종합 판정: PASS. P0/P1 잔여 없음.
- 검수자: 메인 작업자(라이브 렌더) + ux-harness-reviewer subagent

### [2026-06-21] UI 개선 — 새 프로젝트 폼 '광고 세팅' 추천 우선 전환 + 입찰 자동/수동 둘다 — Trigger: 사용자 "세팅 수동입력 말고 추천해라, 입찰 둘다 선택"
- 프로파일: UI(기능 결합)
- 배경: 세팅(반경/성별/연령/일예산/입찰)이 수동 입력 필드라 "추천 도구가 받아적게 한다"는 불만. 입찰 radio라 자동/수동 동시 선택 불가(페어 전략 위배).
- 사용자 결정(AskUserQuestion): Option B(추천 우선 + 선택 기록).
- 수정(`project.py`): ① 세팅 섹션 `ui.expansion`으로 접이식(기본 접힘, "선택 — 비우면 AI가 추천"), 사업 기본정보만 상시 노출. ② 입찰 radio→자동/수동 checkbox 2개(둘 다 가능)+"같이 시작→수동 안정되면 자동 OFF" 안내. ③ "AI 추천 받기" 버튼: budget_planner(비용0 룰엔진)로 일예산·자동+수동 입찰 채움+추천 구조 표시, 성별·연령·반경은 추측 안 하고 기획 단계 안내(정직). _collect/_prefill 체크박스 대응(bid_type comma 문자열, DB 스키마 동일).
- 게이트: **402 passed**(회귀 0). compile OK. 라이브: 기본 접힘 → 펼침 → 일예산 2만 입력 → AI추천 클릭 시 추천구조 표시+자동/수동 둘다 체크+반경3 채움(10/10 자동검증 PASS, 스크린샷).
- 검수: ux-harness-reviewer=**PASS**(P0/P1 없음).
- 종합 판정: PASS. P0/P1 잔여 없음.
- 검수자: 메인 작업자(라이브) + ux-harness-reviewer subagent
