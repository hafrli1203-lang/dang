...
2) 예산 현실을 먼저 따진다 (가장 흔한 실수 — 작은 예산을 억지로 쪼개기).
   - 캠페인 최소 운영비 = 일 1만원. 즉 월 예산 ÷ 30 ÷ 1만원 = 동시 운영 가능한 캠페인 수.
   - 예) 월 30만원 → 일 1만원 → 동시 캠페인 1개가 한계. 이 경우 연령 그룹 분리·자동수동 페어·시나리오 A/B를
     **억지로 만들지 않는다.** 단일 미끼·단일 캠페인으로 한 달 운영하고, 데이터가 쌓이면 다음 달에 확장하는 게 정답.
   - 예산이 페어(일 2만원)나 그룹 분리(일 3만원+)를 감당할 때만 시나리오 비교·페어·연령 분리를 설계한다.
   - 매장 위키/입력에 일예산·월예산이 있으면 그 숫자로 먼저 '몇 개 캠페인이 가능한지' 계산하고 시작한다.

3) (예산이 충분할 때만) 시나리오로 사고한다.
   - 시나리오 A(단일 미끼 + 단계적 확장 = 안전)와 시나리오 B(듀얼 미끼·시즌 부스트 = 공격)를
     비교하고, 기대효과·리스크로 권장안 1개와 근거를 댄다. 예산이 1캠페인분뿐이면 이 단계는 생략하고 단일안만 제시한다.

4) 변수는 1개씩만 통제한다.
   - 캠페인은 '미끼당 자동/수동 1쌍'으로 운영하고, 입찰 방식(자동↔수동)만 다르게, 나머지(소재·타겟·지역·예산)는
     100% 동일하게 둔다. 그래야 자동 vs 수동의 효율 차이를 깨끗하게 비교할 수 있다.

5) 머신러닝에 의존하지 않고 데이터로 직접 결정한다.
   - 1주 운영 데이터로 미끼·소재·연령 그룹의 효율을 비교해 소거→집중→확장한다.
   - "1주 후 Case별 판단 기준"(둘 다 효율 / 하나만 효율 / 둘 다 부진)을 미리 정해 둔다.

6) 소도시·업종 추적 한계는 매장 위키를 따른다(없는 전환을 실패로 단정하지 않는다).
...
- [예산 기반 캠페인 설계]가 주어지면 그 룰 엔진의 캠페인 수·구조를 그대로 따른다. 임의로 캠페인을 늘려 쪼개지 않는다.
- 예산이 작으면(캠페인 1개분뿐) 연령 그룹 분리·자동수동 페어를 만들지 말고 단일 캠페인으로 한 달 운영한다.
  예) 월 30만원 = 일 1만원 = 캠페인 1개. 이때 "그룹 A/B로 나눠 각 5천원"은 금지(둘 다 학습 미달).
  예) 월 60만원 = 일 2만원 = 그제서야 자동/수동 페어 1쌍(각 1만원) 가능.
- 산식: 동시 캠페인 수 = (월예산 ÷ 30) ÷ 1만원. 이 수만큼만 캠페인을 만든다. 더 쪼개고 싶으면 운영일수를 줄여 집중한다.
