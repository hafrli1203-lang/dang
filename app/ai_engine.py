...
[분석 프레임워크 — 퍼널 기반 사고법]
- 당근 광고 퍼널: 광고노출 -> 콘텐츠클릭 -> 채팅문의 -> 단골맺기 -> 쿠폰사용
- 각 단계별 전환율을 분석하고, 이탈이 가장 큰 구간을 핵심 개선 포인트로 삼는다.
- 성과 최적화 순서: 콘텐츠(소식글/카피) -> 캠페인(타겟/기간) -> 타겟(연령/지역) -> 예산(증감)
- 기간별 데이터가 있으면 효율/비효율 기간을 분류하고 예산 재배분을 제안한다.
- 소거->집중->스케일업 사이클을 따른다.
...
- 퍼널 전환율 흐름과 수익성(CPA 기준)을 반드시 언급한다.
...
- 성과 최적화 순서(콘텐츠->캠페인->타겟->예산)를 따라 우선순위를 매긴다.
- 퍼널에서 이탈이 가장 큰 구간에 대한 개선안을 반드시 포함한다.
- 막연한 제안("더 노력하세요") 금지.

## 3. 잘 된 것
- 1~3줄로 데이터가 보여주는 긍정적 사실만 서술한다.
- 추측이나 과장 없이 데이터 근거를 포함한다.
- 퍼널에서 전환율이 높은 단계가 있으면 언급한다.

## 4. 막힌 것
- 1~3줄로 개선이 필요한 부분, 목표 미달 지표 등을 서술한다.
- 퍼널 이탈 구간(전환율이 가장 낮은 단계)을 구체적으로 명시한다.
- 문제 원인에 대한 가설이 있으면 짧게 언급한다.

## 5. 가설
- 1~3줄로 성과 변동의 원인 추정이나 테스트할 가설을 제시한다.
- 퍼널 이탈 원인에 대한 가설을 포함한다.
- 검증 가능한 형태로 서술한다.
...
- 비효율 기간 OFF, 효율 기간 예산 증액 등 구체적 실험을 포함한다.
- 구체적이고 실행 가능한 실험을 제안한다.

## 7. 판단 기준
- 아래 세 가지를 각 1문장으로 작성한다:
  확대: (CPA가 목표 이하이고 전환율 유지 시)
  검토: (CPA가 목표 근처이거나 전환율 하락 추세 시)
  중단: (CPA가 목표 초과이고 개선 실험 소진 시)
...
## 1. 결론
- 퍼널 전환율(노출->클릭->문의->단골->쿠폰)을 기반으로 전체 성과를 평가한다.
- 수익성 판단: 현재 CPA vs MAX CPA 비율로 확장 여력을 언급한다.

## 2. Next Actions
- 성과 최적화 순서(콘텐츠->캠페인->타겟->예산)를 따라 제안한다.
- 퍼널에서 이탈이 가장 큰 구간에 대한 구체적 개선안을 포함한다.

## 3. 잘 된 것
## 4. 막힌 것
- 퍼널 이탈 구간(전환율이 가장 낮은 단계)을 명시한다.

## 5. 가설
- 퍼널 이탈 원인에 대한 가설을 제시한다.

## 6. 다음 실험
- 비효율 기간 OFF 또는 효율 기간 예산 증액 등 구체적 실험을 포함한다.

## 7. 판단 기준
- CPA/MAX CPA 비율을 기준으로 확대/검토/중단 조건을 제시한다.
"""
...
## 3. 전략 방향
- 추천 전략 타입 (진정성/긴급성/가성비) + 근거
- 핵심 메시지 방향
- 소구 포인트 우선순위

## 4. 캠페인 그룹 구성
- 나이대별 캠페인 그룹 (최소 2개)
- 각 그룹 네이밍 규칙 (지역_비즈타겟_키워드)
- 그룹별 핵심 소구점 + 예산 배분 비율
"""
