...
📍 위치: {{region}} (상세 주소가 매장 정보에 있으면 그대로, 없으면 동네까지만 자연스럽게)
🕐 영업시간 / 🅿 주차: (입력에 있으면 표기. 없으면 이 줄은 생략하고 아래 채팅 안내로 대체 — "[매장 확인 필요]" 같은 미완성 표시를 본문에 남기지 말 것)
📞 정확한 위치·영업시간은 채팅으로 안내드려요 / "당근 보고 왔어요" 한마디면 됩니다
...
(구체적 가격·숫자를 본문에 최소 1회 명시: 입력 데이터의 실제 가격을 그대로 쓴다. 가격을 임의로 부풀리거나 지어내지 말 것)
(입력 혜택이 여러 개면 메인 1개 외 보조 혜택도 구분선으로 묶어 함께 노출)
(총 15~25줄, 2~4줄마다 빈 줄로 구분)
(CTA를 본문 중간과 끝에 자연스럽게 배치)
